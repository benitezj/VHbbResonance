...
bool configWHmnubb_13TeV(Plotter * analysis, TString inpath){  
  ...
  if(!analysis->loadBackgroundSamples("Z_l","ZmumuMassiveCB*CVetoBVeto",inpath)) return 0;
  ...
  if(!analysis->loadBackgroundSamples("Z_c","ZmumuMassiveCB*CFilterBVeto",inpath)) return 0;
  ...
  if(!analysis->loadBackgroundSamples("Z_b","ZmumuMassiveCB*BFilter",inpath)) return 0;
  ...
  if(!analysis->loadBackgroundSamples("W_l","WmunuMassiveCBP*CJetVetoBVeto",inpath)) return 0;
  ...
  if(!analysis->loadBackgroundSamples("W_c","WmunuMassiveCB*CJetFilterBVeto",inpath)) return 0;
  ...
  if(!analysis->loadBackgroundSamples("W_b","WmunuMassiveCB*BFilter",inpath)) return 0;
  ...
  //if(skim.CompareTo("")!=0 && !(analysis->skimSamples(skim.Data()))) return 0;

  analysis->setSkimSelection(skim);
  ...
  analysis->plot("vh_mass[vh]/1000",25,0,2500,""," VH mass [GeV]","",c2,"vh_mass",1);
