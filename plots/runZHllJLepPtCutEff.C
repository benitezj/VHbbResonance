...
void runZHllJLepPtCutEff(TString channel, TString intag,TString tag=""){

  TString region="presel";//preselection without Z mass selection

  TString inpath=inputDir_+"/"+channel+"/"+intag;
  TString outpath=outputDir_+"/"+channel+"/LepPtCutEff_"+intag+"_"+region+"_"+tag;
  ...
  if(channel.CompareTo("ZHmmJ")==0)
    CUT="(muo_pt[mm_leg2[vh_v[vh]]]>25000)";

  if(channel.CompareTo("ZHeeJ")==0)
    CUT="(ele_pt[ee_leg2[vh_v[vh]]]>25000)";
  ...
  for(long M=1000;M<=5000;M+=100){
    ...
    if(M%500==0){
      TH1D* HZM = NULL;
      if(channel.CompareTo("ZHmmJ")==0)
	HZM = S.getHistoNtpFile("muo_pt[mm_leg2[vh_v[vh]]]/1000",100,0,1000,TCut(selection));
      if(channel.CompareTo("ZHeeJ")==0)
	HZM = S.getHistoNtpFile("ele_pt[ee_leg2[vh_v[vh]]]/1000",100,0,1000,TCut(selection));
      HZM->SetName(TString("SubleadLepPt")+M);
  ...
  HSubleadLepPt[0]->GetXaxis()->SetTitle("Lepton pT   [GeV]");
